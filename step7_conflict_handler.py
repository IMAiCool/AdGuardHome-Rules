...
# 读取黑白名单
blacklist, black_index = load_lines_with_index('./ipombaw/BlackList_tmp.txt')
whitelist, white_index = load_lines_with_index('./ipombaw/WhiteList_tmp.txt')
...
# 写入临时文件
with open('./TMP/White_list_tmp.txt', 'w', encoding='utf-8') as f:
    f.write('\n'.join(new_whitelist))
with open('./TMP/Black_list_tmp.txt', 'w', encoding='utf-8') as f:
    f.write('\n'.join(new_blacklist))
with open('./Log/both-in-white-and-black.log', 'w', encoding='utf-8') as f:
    f.write('\n\n'.join(both_log))
...
detect_hierarchy_conflict(
    new_blacklist,
    'Black_list_tmp.txt',
    './output/AdBlackList.txt',
    './Log/Hierarchy_conflict.log'
)

detect_hierarchy_conflict(
    new_whitelist,
    'White_list_tmp.txt',
    './output/AdWhiteList.txt',
    './Log/Hierarchy_conflict.log'
)
